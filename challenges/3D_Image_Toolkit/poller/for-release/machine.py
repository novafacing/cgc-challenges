...
class Poller(Actions):
    ...
    def start(self):
        ...
        self.dll = CDLL(root + "/lib3D_Image_Toolkit.so")
    ...
    def rotateImage(self):
        ...
        for px in self.file_data:
            if coord[0] is "x":
                px["y"] = c_short(
                    multiply(px["y"], cos(a)) - multiply(px["z"], sin(a))
                ).value
                px["z"] = c_short(
                    multiply(px["y"], sin(a)) + multiply(px["z"], cos(a))
                ).value

            elif coord[0] is "y":
                px["x"] = c_short(
                    multiply(px["z"], sin(a)) + multiply(px["x"], cos(a))
                ).value
                px["z"] = c_short(
                    multiply(px["z"], cos(a)) - multiply(px["x"], sin(a))
                ).value

            elif coord[0] is "z":
                px["x"] = c_short(
                    multiply(px["x"], cos(a)) - multiply(px["y"], sin(a))
                ).value
                px["y"] = c_short(
                    multiply(px["x"], sin(a)) + multiply(px["y"], cos(a))
                ).value
