...
class MyClass(Actions):
    ...
    def InitCustomMaterial(self):
        ...
        for temp in temps:
            ...
            rnd = rnd * random.uniform(0.75, 1.25)
            rnd_text = "%0.5f" % rnd
            self.write(rnd_text + "\n")
    ...
    def InitMaterial(self):
        ...
        if random.randint(0, 1) == 1:
            ...
            self.read(
                delim=": ",
                expect="  What temperature? (-173.15000 - 373.00000 degrees C): ",
            )
        ...
        self.read(
            delim=": ",
            expect="Are there any constant energy sources in the room? (y,n): ",
        )
        if random.randint(0, 1) == 1:
        ...
        self.read(
            delim=": ",
            expect="For how long would you like to run the simulation? (s): ",
        )
        ...
        for z in range(0, self.Sim.Z.value):
            ...
            while starty < self.Sim.Y.value:
                ...
                while startx < self.Sim.X.value:
                    output[x][y] = self.Sim.TGridAverage(
                        x, y, startx, starty, countx, county, z
                    )
                    startx += countx
                    x += 1
