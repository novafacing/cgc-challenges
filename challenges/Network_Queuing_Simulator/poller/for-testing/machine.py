...
class Mersenne:
    def __init__(self, seed):
        ...
        for i in range(1, 624):
            self.mt_state[i] = (
                0x6C078965 * (self.mt_state[i - 1] ^ (self.mt_state[i - 1] >> 30)) + i
            ) & 0xFFFFFFFF

    def gen_mt_state(self):
        for i in range(624):
            t = (self.mt_state[i] & 0x80000000) + (
                self.mt_state[(i + 1) % 624] & 0x7FFFFFFF
            )
...
class Generator:
    def __init__(self, MT):
        ...
        self.cgc_NextPkt = {}
        self.cgc_NextPkt["timestamp"] = 0.0
        self.cgc_NextPkt["bytes"] = 0
        self.cgc_NextPkt["priority"] = 0
    ...
    def GenerateRandom(self):
        if self.cgc_NextPkt["bytes"]:
            return self.cgc_NextPkt
        ...
        self.cgc_NextPkt["timestamp"] = self.last_pkt_time + delta
        self.last_pkt_time = self.cgc_NextPkt["timestamp"]
        self.cgc_NextPkt["bytes"] = length
        self.cgc_NextPkt["priority"] = pri
    ...
    def GenerateManual(self):
        ...
        if self.ManualCurr == -1:
            ...
            self.cgc_NextPkt["timestamp"] = (
                self.ManualPkts[0]["timestamp"] + self.last_pkt_time
            )
            self.last_pkt_time = self.cgc_NextPkt["timestamp"]
            self.cgc_NextPkt["bytes"] = self.ManualPkts[0]["bytes"]
            self.cgc_NextPkt["priority"] = self.ManualPkts[0]["priority"]
            ...
            self.cgc_NextPkt["timestamp"] = (
                self.ManualPkts[self.ManualCurr]["timestamp"] + self.last_pkt_time
            )
            self.last_pkt_time = self.cgc_NextPkt["timestamp"]
            self.cgc_NextPkt["bytes"] = self.ManualPkts[self.ManualCurr]["bytes"]
            self.cgc_NextPkt["priority"] = self.ManualPkts[self.ManualCurr]["priority"]
...
class NetSim(Actions):
    def start(self):
        ...
        self.cgc_iface = {}
        self.cgc_iface["speed"] = 0
        self.cgc_iface["total_pkts"] = 0
        self.cgc_iface["total_bytes"] = 0
        self.cgc_iface["num_queues"] = 0
        self.cgc_iface["priority_queue_enable"] = 0
        self.cgc_iface["ifg"] = 0.0

        self.generator = 0
        self.gen_rate = 0.0
        self.repeat = 0
        self.cgc_wall_clock = 0.0
    ...
    def SetIntSpeed(self):
        ...
        self.read(
            delim=": ", expect="What's the interface speed (in bps up to 4194304 bps): "
        )
        ...
        if random.randint(0, 1):
            ...
            self.read(
                delim=": ",
                expect="What's the interface speed (in bps up to 4194304 bps): ",
            )

        # send valid input
        self.cgc_iface["speed"] = random.randint(10000, 4194304)
        self.write("%d\n" % self.cgc_iface["speed"])
    ...
    def SetGenerator(self):
        ...
        if random.randint(0, 1):
            ...
            self.write("%d\n" % self.generator)
            self.read(
                delim=": ", expect="Which generator would you like to use? (1-3): "
            )
        ...
        self.read(
            delim=": ",
            expect="For how long would you like the simulation to run? (1 - 10s): ",
        )
        self.cgc_max_wall_clock = random.randint(0, 10)
        self.write("%d\n" % self.cgc_max_wall_clock)
    ...
    def SetRandomGenerator(self):
        self.read(
            delim=": ",
            expect="What average bit rate do you want the random generator to produce? (1 - 4194304 bps): ",
        )
        # randomly try sending an incorrect value
        if random.randint(0, 1):
            # send an invalid input
    ...
    def SetPoissonGenerator(self):
        self.read(
            delim=": ",
            expect="What average bit rate do you want the poisson generator to produce? (1 - 4194304 bps): ",
        )
        # randomly try sending an incorrect value
        if random.randint(0, 1):
            # send an invalid input
    ...
    def SetManualGenerator(self):
        self.read(
            delim=": ",
            expect="Should the packet generator repeat the manually entered packets? (y,n): ",
        )
        # randomly try sending an incorrect value
        ...
        self.read(
            delim=": ", expect="How many packets would you like to enter? (1 - 1000): "
        )

        # randomly try sending an incorrect value
        if random.randint(0, 50) == 0:
            # send an invalid input
            pkt_count = random.choice(
                [random.randint(-9999, 0), random.randint(1001, 9999)]
            )
            self.write("%d\n" % pkt_count)
            self.read(
                delim=": ",
                expect="How many packets would you like to enter? (1 - 1000): ",
            )
        ...
        if choice > 40:
            ...
            self.write(
                ...
                % (
                    time,
                    random.choice(
                        [random.randint(-10, 63), random.randint(1501, 9999)]
                    ),
                    priority,
                )
            )
            ...
            self.write(
                ...
                % (
                    time,
                    length,
                    random.choice([random.randint(-10, -1), random.randint(64, 99)]),
                )
            )
    ...
    def SetQueues(self):
        ...
        for q in range(0, self.num_queues):
            ...
            self.read(
                delim=": ",
                expect="  What's the minimum priority packet to place in queue #%d (0-63): "
                % q,
            )
            ...
            self.read(
                delim=": ",
                expect="  What's the maximum priority packet to place in queue #%d (0-63): "
                % q,
            )
            ...
            self.read(
                delim=": ",
                expect="  What is the depth of queue #%d (1 - 1024 packets): " % q,
            )
            ...
            if q == 0 and self.cgc_iface["priority_queue_enable"]:
                ...
                self.read(
                    delim=": ",
                    expect="  What is the weight of queue #%d (1 - 100 percent): " % q,
                )
        ...
        for q in range(0, self.num_queues):
            self.queue[q]["max_tokens"] = int(
                total_tokens * (self.queue[q]["weight"] / 100.0)
            )
    ...
    def RX(self):
        ...
        while p["bytes"]:
            if p["timestamp"] > self.cgc_wall_clock:
            ...
            for q in range(0, self.num_queues):
                if (
                    p["priority"] >= self.queue[q]["min_priority"]
                    and p["priority"] <= self.queue[q]["max_priority"]
                ):
    ...
    def TX(self):
        ...
        if self.cgc_iface["priority_queue_enable"]:
            while self.queue[0]["curr_depth"]:
                # get the next packet and send it
                p = self.queue[0]["pkts"].popleft()
                self.queue[0]["cumulative_latency"] += (
                    self.cgc_wall_clock - p["timestamp"]
                )
                self.cgc_wall_clock += (
                    (p["bytes"] * 8.0) / self.cgc_iface["speed"]
                ) + self.cgc_iface["ifg"]
                ...
                self.cgc_iface["total_pkts"] += 1
                self.cgc_iface["total_bytes"] += p["bytes"]
        ...
        for q in range(0, self.num_queues):
            ...
            while self.queue[q]["curr_depth"]:
                ...
                if self.queue[q]["pkts"][0]["bytes"] <= self.queue[q]["bucket"]:
                    ...
                    self.queue[q]["cumulative_latency"] += (
                        self.cgc_wall_clock - p["timestamp"]
                    )
                    self.cgc_wall_clock += (
                        (p["bytes"] * 8.0) / self.cgc_iface["speed"]
    ...
    def CheckStats(self):
        ...
        self.read(
            delim="\n",
            expect_format="pcre",
            expect="cgc_wall_clock:\s+\d+\.\d+\s+\(s\)",
        )
        for q in range(0, self.num_queues):
            self.read(delim="\n", expect="Queue %d\n" % q)
            self.read(
            ...
            if self.queue[q]["total_pkts"] == 0:
                ...
                self.read(
                    delim="\n",
                    expect_format="pcre",
                    expect="  Average Latency \(s\):\s+\d+\.\d+",
                )
