...
class PollGenerator(Actions):
    ...
    def init_prng(self):
        ...
        self.prng = ctypes.CDLL(
            root + "/REMATCH_2--Mail_Server--Crackaddr/libCROMU_00098.so"
        )
    ...
    def sendmail_read_queue(self, mq):
        ...
        for msg in mq["messages"]:
            self.read(
                delim="\n",
                expect="%d) From:%s To: %s SUBJECT:%s"
    ...
    def sendmail_ADDRESSBOOK(self):
        ...
        for each in self.address_book:
            self.read(
                delim="\n",
                expect="%d) %s@%s [%d]"
                % (each["index"], each["name"], each["home_server"], each["mode"]),
            )
    ...
    def sendmail_READ(self):
        ...
        self.read(
            ...
            % (
                msg_num,
                mq["messages"][msg_num]["subject"],
                mq["messages"][msg_num]["data"],
            ),
        )
