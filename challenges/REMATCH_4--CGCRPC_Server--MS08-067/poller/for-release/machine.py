...
class FileSystem():
  ...
  def __init__(self, prng):
    self.trees = []
    self.cgc_serviceTypes = []
    self.prng = prng
    self.specialTree = None
    for i in range(self.MAX_SERVICE_TYPES):
      self.cgc_serviceTypes.append(prng.random_string(prng.random_in_range(self.MAX_SERVICE_NAME/2, self.MAX_SERVICE_NAME)))
...
class CROMU_00095_Generator(Actions):
  ...
  def netstuff_service_enum(self):
    ...
    response.AddData(''.join(self.fs.cgc_serviceTypes), 0)
