...
class FileSystem():
  ...
  def __init__(self, magic_page):
    ...
    self.cgc_root = FileNode("cgc_root", FILE_DIRECTORY, "", 0)
    self.cwd = self.cgc_root
    self.numFiles = 1
    self.allFiles = []
    ...
    self.cgc_root.child = documents
    documents.parent = self.cgc_root
    system = FileNode("System", FILE_DIRECTORY, "", self.getNextFileID())
    documents.next = system
    system.prev = documents
    system.parent = self.cgc_root
    
    system.next = user
    user.prev = system
    user.parent = self.cgc_root
    ...
    self.reservedFiles = ["System", "Special", "Flavor.sl", "cgc_root"]
