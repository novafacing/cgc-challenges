...
class StringSearch(Actions):
    ...
    def add_string(self):
        ...
        string_to_add = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(random.randint(0, 1028)))

        self.write('a/' + string_to_add + '\n')

        if len(string_to_add) > 9:
...
class bst:
    ...
    def calc_token(self, value):
        ...
        # just do the first 10 chars of the string 
        for char in value:
