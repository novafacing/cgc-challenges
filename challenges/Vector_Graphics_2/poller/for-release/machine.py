...
class VGFObjSpray():
    ...
    def generate_random(self, max_x, max_y, bad_data = True):
        ...
        max_width = self.x_start
        if ( max_width > max_x - 1 - self.x_start ):
            max_width = max_x - 1 - self.x_start
        if ( max_width > self.y_start ):
            max_width = self.y_start
        if ( max_width > max_y - 1 - self.y_start ):
            max_width = max_y - 1 - self.y_start 

        self.width = random.randint( 0, max_width )
    ...
    def generate_file_data(self):
        ...
        file_data = struct.pack( 'H', self.x_start )
        file_data += struct.pack( 'H', self.y_start )
        file_data += struct.pack( 'H', self.width )
        file_data += struct.pack( 'H', self.density )
        file_data += struct.pack( 'I', 0x59745974 )
    ...
    def render(self, render_obj, color_index, do_fill ):
        ...
        y_start = self.y_start
        width = self.width
        if (self.density > 100 or self.density == 0):
            return 
        else:
            step = 100 / self.density
        ...
        while ( x_pos < x_start + width ):
            render_obj.draw_pixel( x_pos, y_pos, color_index )
            x_pos += step

        x_pos = x_start
        while (( x_pos > 0 ) and ( x_pos > x_start - width )):
            render_obj.draw_pixel( x_pos, y_pos, color_index )
            x_pos -= step

        x_pos = x_start
        while ( y_pos < y_start + width ):
            render_obj.draw_pixel( x_pos, y_pos, color_index )
            y_pos += step

        y_pos = y_start
        while (( y_pos > 0 ) and ( y_pos > y_start - width)) :
            render_obj.draw_pixel( x_pos, y_pos, color_index )
            y_pos -= step

        y_pos = y_start

        render_obj.draw_pixel( x_pos + width, y_pos, color_index )
        render_obj.draw_pixel( x_pos - width, y_pos, color_index )
        render_obj.draw_pixel( x_pos, y_pos + width, color_index )
        render_obj.draw_pixel( x_pos, y_pos - width, color_index )
...
class VGFObject():
    ...
    def generate_random(self, max_x, max_y, bad_data = False):
        self.object_type = random.randint( 0, 4 )
        ...
        elif ( self.object_type == VGF_OBJECT_SQUARE ):
            self.object_data = VGFObjSquare()
        elif ( self.object_type == VGF_OBJECT_SPRAY ):
            self.object_data = VGFObjSpray()
