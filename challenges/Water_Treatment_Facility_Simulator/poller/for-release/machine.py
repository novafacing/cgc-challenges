...
class MyClass(Actions):
    ...
    def CalculateBestOperatingParameters(self):
        ...
        for cgc_Asp in range(0,3):
            InputValvePct = self.Wtfs.GetValvePctFlow(self.Wtfs.HeadworksOutputValve, ct.c_ubyte(cgc_Asp))
...
#            print "CbodPerHr {:8.2f} {:8.2f} {:8.2f}\n".format(CbodPerHr,TnPerHr,TpPerHr)
            ...
            self.write("{:0.2f}\n".format(PctAeration))
            self.CheckAspMenu()
            self.Wtfs.ChangeAerationPercent(self.Wtfs.cgc_Asp[cgc_Asp], float("{:0.2f}\n".format(PctAeration)))
            ...
            self.write("{:0.2f}\n".format(AlumRate))
            self.CheckAspMenu()
            self.Wtfs.ChangeAlumRate(self.Wtfs.cgc_Asp[cgc_Asp], float("{:0.2f}\n".format(AlumRate)))
	...
	# V2
        for Output in range(0,3):
            ...
            self.write("{:0.2f}\n".format(PctOpen))
            self.CheckValvesMenu()
            self.Wtfs.SetValveOutput(self.Wtfs.cgc_AspOutputValve, Output, PctOpen)
            OutputPct += PctOpen
        if OutputPct < 100.0:
            ValveCheckFail = 1
	# V3
	...
	# ASPs
        ...
        self.write(NewName + "\n")
	self.Wtfs.SetSystemName(self.Wtfs.cgc_Asp[0], NewName);
	self.CheckRenameMenu()
	...
	self.CheckRenameMenu()
        ...
        self.write(NewName + "\n")
	self.Wtfs.SetSystemName(self.Wtfs.cgc_AspOutputValve, NewName);
	self.CheckRenameMenu()
