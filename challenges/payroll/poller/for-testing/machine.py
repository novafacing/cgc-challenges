...
class TimecardPollGenerator(Actions):
    def start(self):
        ...
        testlib = ctypes.CDLL(root + "/payroll/libCROMU_00007.so")
        self.standardpay = testlib.cgc_c_standardpay
        self.standardpay.argtypes = [
            ctypes.c_int,
            ctypes.c_int,
            ctypes.c_int,
            ctypes.c_int,
    ...
    def register(self):
        ...
        for id in self.state["id_subset"]:
            ...
            if self.state["employees"][id].exempt:
                self.write(
                    "employee_id-{a.id}`employee_name-{a.name}`wage-{a.wagedollars}.{a.wagecents:02}`exempt-yes`\n".format(
                        a=self.state["employees"][id]
                    )
                )
            else:
                self.write(
                    "employee_id-{a.id}`employee_name-{a.name}`wage-{a.wagedollars}.{a.wagecents:02}`\n".format(
                        a=self.state["employees"][id]
                    )
                )
    ...
    def loghours(self):
        for id in self.state["id_subset"]:
            for week in self.state["weeks_subset"]:
                ...
                if self.chance(0.9):
                    self.logday(
                        "wednesday", self.state["employees"][id].paychecks[week]
                    )
                if self.chance(0.01):
                    self.logday(
                        "wednesday", self.state["employees"][id].paychecks[week]
                ...
                paycheck.standardpay = self.standardpay(
                    paycheck.standardhours,
                    paycheck.standardminutes,
                    employee.wagedollars,
                    employee.wagecents,
                )
                if employee.exempt:
                    paycheck.overtimepay = self.overtimepay(
                        paycheck.overtimehours,
                        paycheck.overtimeminutes,
                        employee.wagedollars,
                        employee.wagecents,
                        1,
                    )
    ...
    def get_response(self, id, week):
        ...
        response = "employee-{}`week-{}`standardtime-{pay.standardhours}h{pay.standardminutes}m`overtime-{pay.overtimehours}h{pay.overtimeminutes}m`standardpay-${pay.standardpay:,.2f}`overtimepay-${pay.overtimepay:,.2f}`netpay-${netpay:,.2f}`\n".format(
            self.state["employees"][id].name, week, pay=pay, netpay=netpay
        )
        return response
