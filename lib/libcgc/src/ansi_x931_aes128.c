...
void cgc_aes_get_bytes(cgc_prng *prng, uint32_t len, uint8_t *buf) {
  ...
  while (buf_idx < len) {
    ...
    // Copy over data until we run out of data or the end of the buffer is
    // reached
...
/**
 * XORs the first BLOCK_SIZE bytes of two buffers and saves the result in an
 * output buffer
