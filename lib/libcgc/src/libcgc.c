...
int cgc_fdwait(int nfds, cgc_fd_set *readfds, cgc_fd_set *writefds,
  ...
  if (writefds) {
    if (0 !=
        (ret = cgc_copy_cgc_fd_set(writefds, &write_fds, &actual_num_fds))) {
      return ret;
    }
  }
  ...
  int num_selected_fds =
      select(nfds, (readfds ? &read_fds : NULL), (writefds ? &write_fds : NULL),
             NULL, (timeout ? &max_wait_time : NULL));
...
// Math functions
float cgc_sinf(float f) { return sinf(f); }

double cgc_sin(double d) { return sin(d); }

long double cgc_sinl(long double d) { return sinl(d); }

float cgc_cosf(float f) { return cosf(f); }

double cgc_cos(double d) { return cos(d); }

long double cgc_cosl(long double d) { return cosl(d); }

float cgc_tanf(float f) { return tanf(f); }

double cgc_tan(double d) { return tan(d); }

long double cgc_tanl(long double d) { return tanl(d); }

float cgc_logf(float f) { return logf(f); }

double cgc_log(double d) { return log(d); }

long double cgc_logl(long double d) { return logl(d); }

float cgc_rintf(float f) { return rintf(f); }

double cgc_rint(double d) { return rint(d); }

long double cgc_rintl(long double d) { return rintl(d); }

float cgc_sqrtf(float f) { return sqrtf(f); }

double cgc_sqrt(double d) { return sqrt(d); }

long double cgc_sqrtl(long double d) { return sqrtl(d); }

float cgc_fabsf(float f) { return fabsf(f); }

double cgc_fabs(double d) { return fabs(d); }

long double cgc_fabsl(long double d) { return fabsl(d); }

float cgc_log2f(float f) { return log2f(f); }

double cgc_log2(double d) { return log2(d); }

long double cgc_log2l(long double d) { return log2l(d); }

float cgc_exp2f(float f) { return exp2f(f); }

double cgc_exp2(double d) { return exp2(d); }

long double cgc_exp2l(long double d) { return exp2l(d); }

float cgc_expf(float f) { return expf(f); }

double cgc_exp(double d) { return exp(d); }

long double cgc_expl(long double d) { return expl(d); }

float cgc_log10f(float f) { return log10f(f); }

double cgc_log10(double d) { return log10(d); }

long double cgc_log10l(long double d) { return log10l(d); }

float cgc_powf(float f1, float f2) { return powf(f1, f2); }

double cgc_pow(double d1, double d2) { return pow(d1, d2); }

long double cgc_powl(long double d1, long double d2) { return powl(d1, d2); }

float cgc_atan2f(float f1, float f2) { return atan2f(f1, f2); }

double cgc_atan2(double d1, double d2) { return atan2(d1, d2); }

long double cgc_atan2l(long double d1, long double d2) {
  return atan2l(d1, d2);
}

float cgc_remainderf(float f1, float f2) { return remainderf(f1, f2); }

double cgc_remainder(double d1, double d2) { return remainder(d1, d2); }

long double cgc_remainderl(long double d1, long double d2) {
  return remainderl(d1, d2);
}

float cgc_scalbnf(float f, int i) { return scalbnf(f, i); }

double cgc_scalbn(double d, int i) { return scalbn(d, i); }

long double cgc_scalbnl(long double d, int i) { return scalbnl(d, i); }

float cgc_scalblnf(float f, long int i) { return scalblnf(f, i); }

double cgc_scalbln(double d, long int i) { return scalbln(d, i); }

long double cgc_scalblnl(long double d, long int i) { return scalblnl(d, i); }

float cgc_significandf(float f) { return significandf(f); }

double cgc_significand(double d) { return significand(d); }

long double cgc_significandl(long double d) { return significandl(d); }
